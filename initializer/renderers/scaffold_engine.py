...
def _package_json(spec: dict[str, Any]) -> str:
    ...
    pkg: dict[str, Any] = {
        ...
        "scripts": {
            ...
            "start": "next start",
            "lint": "eslint .",
            "test": "tsx --tsconfig tsconfig.test.json --test src/**/*.test.ts src/**/*.test.tsx",
...
test("home page smoke render includes the project title", () => {{
  const html = renderToStaticMarkup(React.createElement(Home));

  assert.ok(html.includes("{name}"));
  assert.ok(html.includes("Project generated by Specwright"));
}});

test("home page module exports a renderable component", () => {{
  assert.equal(typeof Home, "function");
}});
"""
...
def write_scaffold(output_dir: Path, spec: dict[str, Any]) -> None:
    ...
    _write(output_dir, "tsconfig.test.json", _tsconfig_test())
    ...
    _write(output_dir, "src/__tests__/smoke.test.tsx", _smoke_test(spec))
